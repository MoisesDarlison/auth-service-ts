...
import { FilterUserByEmailUserCase } from "./filter-user-by-email.use-case";

describe("Filter User by ID - Use Case", () => {
  it("Should be able Filter user by Email", async () => {
  ...
  it("Should to return NULL if it does not find the user by EMAIL", async () => {
    const userRepoInMemory = new UserInMemoryRepository();
    const userUserCase = new FilterUserByEmailUserCase(userRepoInMemory);
